...
Image 1: A real bacteria undergoing binary fission (Britannica, 2024)

## DNA Replication
Two identical $\text{chromosomes}$ are replicated because we will produce another identical bacterial cell. After the cell grows for a while, it needs to activate the $\text{DNA}$ replication procedure using $\text{DnaA}$, a common $\text{replicon}$ protein for initiating $\text{DNA}$ replication. The chromosome is a $\text{double-stranded circular DNA}$ connected at the origin of replication to form a circle. 

A group of proteins called $\text{replisome}$ travels to the origin of replication and replicates $\text{DNA}$ bidirectionally. The $\text{DNA}$ will extend to the left and right sides of the origin in the circle. $\text{Replisome}$ proteins include $\text{helicase}$, which unpacks genetic information, $\text{primase}$ (synthesizes short $\text{RNA-seq}$), and $\text{DNA}$ $\text{polymerase}$, which creates $\text{DNA}$ molecules using $\text{nucleotides}$ (the foundation substance for $\text{DNA}$ and $\text{RNA}$).  

Under ideal growing conditions (provided in a lab), a bacteria takes an average of 40 minutes to divide. However, some bacteria, like E. coli, only need 20 minutes, as there are two (or more) origins of replication for $\text{DNA}$ cloning. This makes the $\text{DNA}$ replication process happen two times faster. 

More complex substances are involved, but these essential substances allow $\text{DNA}$ replication. 

## Pre-Cytokinesis
After the $\text{DNA}$ is replicated into two bundles of chromosomes, the cell needs to be split exactly in half to retain the "identical" mindset. Hence, a Z-ring and other proteins are formed to aid this process.
...
Before cytokinesis occurs, we must ensure the $\text{DNA}$ bundle is split correctly via the Ftsk protein. The FtsK protein binds to the replisomes and coordinates chromosomes to segregate. The actual segregation will be done using translocase proteins (a group of proteins that assists the movement of other substances). 
...
First, the cell membrane and the cell wall contract using the Z-ring. The two $\text{DNA}$ bundles will split apart using the FtsK protein and translocase proteins. Then, the cytoplasm divides and forms a septum. The Z-ring continues to pull down the cell membrane and the cell wall until the whole cell separates at the septum. 
...
:::info
The tail is hydrophobic while the head is hydrophilic!
:::
